...
ECAD_INLINE void ECellCollection::save(Archive & ar, const unsigned int version) const
{
    ECAD_UNUSED(version)
...
template <typename Archive>
ECAD_INLINE void ECellCollection::load(Archive & ar, const unsigned int version)
{
    ECAD_UNUSED(version)
    boost::serialization::void_cast_register<ECellCollection, ICellCollection>();
    ar & BOOST_SERIALIZATION_BASE_OBJECT_NVP(BaseCollection);
