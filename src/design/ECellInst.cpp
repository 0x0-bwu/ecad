...
void ECellInst::serialize(Archive & ar, const unsigned int version)
{
    ECAD_UNUSED(version)
...
ECellInst::ECellInst(std::string name, CPtr<ILayoutView> refLayout, CPtr<ILayoutView> defLayout)
...
void ECellInst::SetName(std::string name)
{
    EHierarchyObj::SetName(std::move(name));
}

const std::string & ECellInst::GetName() const
{
    return EHierarchyObj::GetName();
}

void ECellInst::SetRefLayoutView(CPtr<ILayoutView> refLayout)
{
    EHierarchyObj::SetRefLayoutView(refLayout);
}

CPtr<ILayoutView> ECellInst::GetRefLayoutView() const
{
    return EHierarchyObj::GetRefLayoutView();
}

void ECellInst::SetDefLayoutView(CPtr<ILayoutView> defLayout)
{
    m_defLayout = defLayout;
}

CPtr<ILayoutView> ECellInst::GetDefLayoutView() const
{
    return m_defLayout;
}

CPtr<ILayoutView> ECellInst::GetFlattenedLayoutView() const
{
    return GetDefLayoutView()->GetCell()->GetFlattenedLayoutView();
}

void ECellInst::SetLayerMap(CPtr<ILayerMap> layerMap)
{
    m_layerMap = layerMap;
}

CPtr<ILayerMap> ECellInst::GetLayerMap() const
{
    return m_layerMap;
}
