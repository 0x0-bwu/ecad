...
void EDefinition::serialize(Archive & ar, const unsigned int version)
{
    ECAD_UNUSED(version)
...
EDefinition::EDefinition(std::string name, CPtr<IDatabase> database)
...
void EDefinition::SetDatabase(CPtr<IDatabase> database)
{
    m_database = database;
}

CPtr<IDatabase> EDefinition::GetDatabase() const
{
    return m_database;
}

EDefinitionType EDefinition::GetDefinitionType() const
{
    ECAD_ASSERT(false)
