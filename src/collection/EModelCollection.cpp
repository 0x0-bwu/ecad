...
ECAD_INLINE void EModelCollection::save(Archive & ar, const unsigned int version) const
{
    ECAD_UNUSED(version)
...
template <typename Archive>
ECAD_INLINE void EModelCollection::load(Archive & ar, const unsigned int version)
{
    ECAD_UNUSED(version)
    boost::serialization::void_cast_register<EModelCollection, IModelCollection>();
    ar & BOOST_SERIALIZATION_BASE_OBJECT_NVP(BaseCollection);
