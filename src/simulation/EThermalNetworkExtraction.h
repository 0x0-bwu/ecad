...
using EThermalNetworkNumType = eutils::EMetalFractionNumType;
...
private:
    void AddBoundaryConditionForLayer(const std::vector<float_t> & iniT, const EGridDataTable & dataTable, EGridThermalModel::BCType type, size_t layer, ThermalNetwork<float_t> & network) const;

private:
    float_t GetMetalComposite(const ESize3D & indexm) const;
    std::array<float_t, 3> GetCompositeMatK(const ESize3D & index, float_t refT) const;
    std::array<float_t, 3> GetConductingMatK(const ESize3D & index, float_t refT) const;
    std::array<float_t, 3> GetDielectricMatK(const ESize3D & index, float_t refT) const;
