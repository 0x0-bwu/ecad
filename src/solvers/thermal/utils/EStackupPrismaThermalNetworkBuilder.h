//todo refactor with EPrismaThermalNetworkBuilder
#pragma once
...
struct EStackupPrismaThermalNetworkBuildSummary
{
    size_t totalNodes = 0;
    size_t fixedTNodes = 0;
    size_t boundaryNodes = 0;
    double iHeatFlow = 0, oHeatFlow = 0;
    void Reset() { *this = EStackupPrismaThermalNetworkBuildSummary{}; }
};

class ECAD_API EStackupPrismaThermalNetworkBuilder
{
public:
    using ModelType = EStackupPrismaThermalModel;
    mutable EStackupPrismaThermalNetworkBuildSummary summary;
    explicit EStackupPrismaThermalNetworkBuilder(const ModelType & model);
    virtual ~EStackupPrismaThermalNetworkBuilder() = default;
